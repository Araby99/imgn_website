...
export default () => {
    ...
    const uploadRest = (content, e) => {
        ...
        axios.post(process.env.NEXT_PUBLIC_CLOUD, formData).then(image => {
            ...
            axios.post("/articles/create", { title, subTitle, hero, description }, { headers: { admin: localStorage.getItem("token") ? localStorage.getItem("token") : sessionStorage.getItem("token") } }).then(res => {
                setLoading(false)
